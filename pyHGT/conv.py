...
from torch_geometric.nn.inits import glorot, uniform, normal
...
class HGTConv(MessagePassing):
    def __init__(self, in_dim, out_dim, num_types, num_relations, n_heads, dropout = 0.2, use_norm = True, use_RTE = True, **kwargs):
        ...
        #uniform(self.relation_att)
        #uniform(self.relation_msg)
        normal(self.relation_att,0,0.05)
        normal(self.relation_msg,0,0.04)
    ...
    def message(self, edge_index_i, node_inp_i, node_inp_j, node_type_i, node_type_j, edge_type, edge_time):
        ...
        '''
        self.res_att1     = torch.zeros(data_size, self.n_heads).to(node_inp_i.device)
        ...
        for source_type in range(self.num_types):
            ...
            for target_type in range(self.num_types):
                ...
                for relation_type in range(self.num_relations):
                    ...
                    self.res_att1[idx] = (q_mat * k_mat).sum(dim=-1) * self.relation_pri[relation_type] / self.sqrt_dk
                    '''
        ...
        #attention
        self.res_att2 = self.res_att1
        self.res_att1 =softmax(self.res_att1, edge_index_i)
        
        #embedding in each layer
        res = res_msg * softmax(self.res_att2, edge_index_i).view(-1, self.n_heads, 1)
...
class GeneralConv(nn.Module):
    ...
    def forward(self, meta_xs, node_type, edge_index, edge_type, edge_time):
        if self.conv_name == 'hgt':
            ...
            self.res_att1 = self.base_conv.res_att1
            self.res_att2 = self.base_conv.res_att2
